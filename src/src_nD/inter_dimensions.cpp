...
void InterDimensions::computePairsAndMatch(vector<Cube>& ctr0, vector<Cube>& ctr1, vector<Cube>& ctrComp) {
	...
	while (computeDim > 0) {
#ifdef RUNTIME
		if (computeDim != cgc0.dim-2) { cout << endl << endl;}
		...
		pivotColumnIndex.clear();
		cache.clear();
		if (computeDim != cgc0.dim-2) { matchMap1.clear(); }
		computePairs(ctr1, 1);
#ifdef USE_CLEARING_DIM0
		ctr1.clear();
		...
		pivotColumnIndex.clear();
		cache.clear();
		if (computeDim != cgc0.dim-2) { isMatchedComp.clear(); }
		...
		pivotColumnIndex.clear();
		cache.clear();
		if (computeDim != cgc0.dim-2) { matchMapIm0.clear(); }
#ifdef USE_CLEARING_DIM0
		...
		pivotColumnIndex.clear();
		cache.clear();
		if (computeDim != cgc0.dim-2) { matchMapIm1.clear(); }
#ifdef USE_CLEARING_DIM0
			...
			if (!cacheHit) {
				...
				while (enumerator.hasNextFace()) {
					...
					if (checkEmergentPair && ctr[j].birth == enumerator.nextFace.birth) {
						if (pivotColumnIndex.find(cgc.getCubeIndex(enumerator.nextFace)) == pivotColumnIndex.end()) {
...
#ifdef USE_EMERGENT_PAIRS
					if (checkEmergentPair && ctr[j].birth == enumerator.nextFace.birth) {
						if (pivotColumnIndex.find(cgcComp.getCubeIndex(enumerator.nextFace)) == pivotColumnIndex.end()) {
							pivot = enumerator.nextFace;
                            foundPair = true;
							break;
						} else { checkEmergentPair = false; }
			...
			if (pivot.coordinates[0] != NONE) {
				...
				if (pair != pivotColumnIndex.end()) {
					...
					if (pivot.birth != ctr[i].birth) {
						...
						isMatchedComp.emplace(cgcComp.getCubeIndex(ctr[i]), true);
					}
					break;
				}
			} else {
		...
		while (true) {
			...
			if (pivot.coordinates[0] != NONE) {
				...
				if (pair != pivotColumnIndex.end()) {
					...
					pivotColumnIndex[cgc.getCubeIndex(pivot)] = i;
					if (isMatchedComp[cgcComp.getCubeIndex(ctr[i])]) { matchMapIm.emplace(cgcComp.getCubeIndex(ctr[i]), pivot); }
					break;
				}
			} else {
				ctr[i].coordinates[0] = NONE;
