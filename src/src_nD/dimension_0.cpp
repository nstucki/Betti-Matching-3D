    ...
    for (Cube &edge : edges) {
        ...
        if (parentIdx0 != parentIdx1) {
            ...
            if (birth != edge.birth) {
                ...
                pairsComp.push_back(Pair(
                    Cube(birth, ufComp.getCoordinates(birthIdxComp)), edge));
#endif
...
void Dimension0::enumerateEdges(const CubicalGridComplex &cgc,
                                vector<Cube> &edges) const {
#ifdef RUNTIME
    cout << "enumeration ";
    auto start = high_resolution_clock::now();
#endif
