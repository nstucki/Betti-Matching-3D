...
TopDimension::TopDimension(const CubicalGridComplex &_cgc0,
...
void TopDimension::computePairsAndMatch(vector<Cube> &ctr0, vector<Cube> &ctr1,
                                        vector<Cube> &ctrComp) {
...
void TopDimension::enumerateDualEdges(const CubicalGridComplex &cgc,
                                      vector<Cube> &dualEdges) const {
#ifdef RUNTIME
    cout << "enumeration ";
...
void TopDimension::computeImagePairs(vector<Cube> &dualEdges, uint8_t k) {
    ...
    for (auto edge = dualEdges.rbegin(), last = dualEdges.rend(); edge != last;
        ...
        if (parentIdx0 != parentIdx1) {
            ...
            if (edge->birth != birth) {
                pairs.push_back(
                    Pair(*edge, Cube(birth, uf.getCoordinates(birthIdx))));
    ...
    auto new_end =
        remove_if(dualEdges.begin(), dualEdges.end(),
                  [](const Cube &edge) { return edge.coordinates[0] == NONE; });
    ...
    for (auto edge = dualEdges.rbegin(), last = dualEdges.rend(); edge != last;
        ...
        if (parentIdx0 != parentIdx1) {
            ...
            if (edge->birth != birth) {
                ...
                pairsComp.push_back(
                    Pair(*edge, Cube(birth, ufComp.getCoordinates(birthIdx))));
#endif
                ...
                if (find0 != matchMap0.end() && find1 != matchMap1.end()) {
                    ...
                    isMatched0.emplace(cgc0.getCubeIndex(find0->second.birth),
                                       true);
                    isMatched1.emplace(cgc1.getCubeIndex(find1->second.birth),
                                       true);
                }
            }
            edge->coordinates[0] = NONE;
