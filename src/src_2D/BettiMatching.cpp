            ...
            for (size_t i = 0; i < matches[d].size(); i++) {
                ...
                if (cgc0.shape[0] < 10 && cgc0.shape[1] < 10 &&
                    ...
                    pair<dim2::RepresentativeCycle, dim2::RepresentativeCycle>
                        reprCycles = getMatchedRepresentativeCycles(d, i);
...
dim2::RepresentativeCycle BettiMatching::getUnmatchedRepresentativeCycle(
    const uint8_t &input, const uint8_t &dim, const size_t &index) {
    if (dim >= 2) {
        throw runtime_error("Invalid value for dim");
    }
    if (input >= 2) {
