    ...
    if (unmatchedPairsIndices.has_value()) {
        ...
        for (auto pair = pairs.begin(); pair != pairs.end() && nextUnmatchedPairIndex != unmatchedPairsIndices->end(); pair++) {
            ...
            if (*nextUnmatchedPairIndex <= unmatchedPairsSeen) {
                throw invalid_argument("Unmatched pair indices must be in strictly ascending order");
            }
        }
        ...
        for (auto &pair : pairs) {
            // If no specific unmatched pairs were requested, add all unmatched pairs
            if (!isMatched[pair.birth.index]) {
                requestedPairs.push_back(pair);
            }
        }
...
// Version of above function for the comparison image, where no list of matches and no isMatched map are available
template<typename Pair>
vector<std::reference_wrapper<Pair>> assembleRequestedComparisonPairs(const optional<vector<size_t>> &matchedPairsIndices, vector<Pair> &pairs,
    ...
    // if (matchedPairsIndices.has_value()) {
    //     // We need to iterate over all pairs to find the requested ones since no list of matches is available for comparison pairs
    //     auto nextMatchedPairIndex = matchedPairsIndices->begin();
    //     int matchedPairsSeen = 0;
    //     for (auto pair = pairs.begin(); pair != pairs.end() && nextMatchedPairIndex != matchedPairsIndices->end(); pair++) {
    //         if (isMatched[pair->birth.index]) {
    ...
    //                 throw invalid_argument("Matched pair indices must be in strictly ascending order for the computation on the comparison image");
    //             }
    //         }
    //     }
    //     if (nextMatchedPairIndex != matchedPairsIndices->end()) {
    //         throw invalid_argument("Index " + to_string(*nextMatchedPairIndex) + " is out of bounds for matched pairs");
    //     }
    // } else {
    //     // If no specific matched pairs were requested, add all matched pairs
    //     for (auto &pair : pairs) {
