...
Dimension1::Dimension1(const CubicalGridComplex &_cgc0,
    ...
    : cgc0(_cgc0), cgc1(_cgc1), cgcComp(_cgcComp), config(_config),
      ...
      matchMap0(_cgc0.shape), matchMap1(_cgc0.shape), matchMapIm0(_cgc0.shape),
      matchMapIm1(_cgc0.shape),
#ifdef USE_REDUCTION_MATRIX
      reductionMatrix(_cgc0.shape),
#endif
      pivotColumnIndexInput0(_cgc0.shape), pivotColumnIndexInput1(_cgc1.shape),
      pivotColumnIndexComp(_cgcComp.shape), pivotColumnIndexImage0(_cgc0.shape),
...
#if defined(USE_APPARENT_PAIRS_COMP) or defined(USE_CLEARING_DIM0) or          \
    defined(USE_ISPAIRED)
...
#if not(defined(USE_APPARENT_PAIRS_COMP) or defined(USE_CLEARING_DIM0) or      \
        defined(USE_ISPAIRED))
    comparisonFuture.wait();
#endif
#else
...
RepresentativeCycle
Dimension1::getRepresentativeCycle(const Pair &pair,
    ...
    auto &pivotColumnIndex =
        pivotColumnIndexInput0; // Use input 0 pivot column index (doesn't
                                // really matter which one for this method)
                ...
                if (it.has_value()) {
                    ...
                    if (pivot == pair.birth) {
                        ...
                        while (!workingBoundary.empty()) {
                            ...
                            workingBoundary.pop();
                            if (!workingBoundary.empty() &&
                                c == workingBoundary.top()) {
                                workingBoundary.pop();
                            } else {
                                vertex = {c.x(), c.y(), c.z()};
                                if (find(reprCycle.begin(), reprCycle.end(),
                                         vertex) == reprCycle.end()) {
...
Dimension1::getAllRepresentativeCycles(uint8_t input, bool computeMatchedCycles,
                                       bool computeUnmatchedCycles) {
#ifndef USE_CACHE
    throw runtime_error("computeAllRepresentativeCycles() can only be used "
                        "when compiled with USE_CACHE defined");
#endif
    ...
    if (computeMatchedCycles) {
        for (auto &match : matches) {
            ...
            if (!cachedBoundary.has_value()) {
                throw runtime_error(
                    "A boundary that is needed to get all cycles was deleted "
                    "from cache! Consider increasing the cache size limit.");
            }
    ...
    if (computeUnmatchedCycles) {
        for (auto &pair : pairs) {
            if (!isMatched[pair.birth.index]) {
                ...
                if (!cachedBoundary.has_value()) {
                    throw runtime_error(
                        "A boundary that is needed to get all cycles was "
                        "deleted from cache! Consider increasing the cache "
                        "size limit.");
                }
...
void Dimension1::enumerateEdges(vector<Cube> &edges,
                                const CubicalGridComplex &cgc,
...
#ifdef USE_CACHE
bool Dimension1::columnIsCached(const Cube &column, CubeQueue &workingBoundary,
...
bool Dimension1::isEmergentPair(const Cube &column, Cube &pivot, size_t &j,
    ...
    auto birth =
        (computePairsMode == IMAGE_PAIRS)
            ? cgc.getBirth(column.x(), column.y(), column.z(), column.type(), 2)
            : column.birth;
...
void Dimension1::computePairsUnified(vector<Cube> &ctr, uint8_t k
    ...
    vector<Pair> &pairs = (computePairsMode == COMPARISON_PAIRS)
                              ? pairsComp
                              : ((k == 0) ? pairs0 : pairs1);
    CubeMap<1, Pair> &matchMap = (computePairsMode == INPUT_PAIRS)
                                     ? ((k == 0) ? matchMap0 : matchMap1)
                                     : matchMap0;
    CubeMap<1, uint64_t> &matchMapIm =
        (computePairsMode == IMAGE_PAIRS)
