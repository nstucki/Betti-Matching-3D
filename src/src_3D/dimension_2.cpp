...
Dimension2::Dimension2(const CubicalGridComplex &_cgc0,
    ...
    : cgc0(_cgc0), cgc1(_cgc1), cgcComp(_cgcComp), config(_config),
      ...
      uf0(UnionFindDual(cgc0)), uf1(UnionFindDual(cgc1)),
      ufComp(UnionFindDual(cgcComp)) {}
...
RepresentativeCycle extractRepresentativeCycle(const Pair &pair,
                                               const CubicalGridComplex &cgc,
                                               UnionFindDual &uf) {
    set<tuple<index_t, index_t, index_t>> cubeCoordinates;
    index_t parentIdx0 = uf.find(pair.death.x() * cgc.m_yz +
                                 pair.death.y() * cgc.m_z + pair.death.z());
    for (size_t i = 0; i < cgc.getNumberOfCubes(3); ++i) {
        index_t parentIdx1 = uf.find(i);
        if (parentIdx0 == parentIdx1) {
            cubeCoordinates.insert(vectorToTuple<3>(uf.getCoordinates(i)));
        }
    }
        ...
        for (uint8_t x = 0; x < 2; ++x) {
            for (uint8_t y = 0; y < 2; ++y) {
                for (uint8_t z = 0; z < 2; ++z) {
                    boundaryVertices.insert({std::get<0>(c) + x,
                                             std::get<1>(c) + y,
                                             std::get<2>(c) + z});
                }
            }
...
Dimension2::getAllRepresentativeCycles(uint8_t input, bool computeMatchedCycles,
                                       bool computeUnmatchedCycles) const {
    ...
    for (auto edge = dualEdges.rbegin(), last = dualEdges.rend(); edge != last;
        ...
        if (currentPair != pairs.end() && *edge == currentPair->birth) {
            RepresentativeCycle reprCycle =
                extractRepresentativeCycle(*currentPair, cgc, uf);
    ...
    if (computeMatchedCycles) {
        for (auto &match : matches) {
            ...
            matchedCycles.emplace_back(
                std::move(matchedCyclesByBirth[pair.birth.index]));
        }
    }
...
RepresentativeCycle
Dimension2::getRepresentativeCycle(const Pair &pair,
    ...
    for (auto edge = dualEdges.rbegin(), last = dualEdges.rend(); edge != last;
        ...
        if (parentIdx0 != parentIdx1) {
            ...
            if (edge->birth != birth) {
                ...
                pairs.push_back(Pair(*edge, Cube(birth, birthCoordinates[0],
                                                 birthCoordinates[1],
                                                 birthCoordinates[2], 0)));
    ...
    for (auto edge = dualEdges.rbegin(), last = dualEdges.rend(); edge != last;
        ...
        if (parentIdx0 != parentIdx1) {
            ...
            if (edge->birth != birth) {
                ...
                pairsComp.push_back(Pair(*edge, Cube(birth, birthCoordinates[0],
                                                     birthCoordinates[1],
                                                     birthCoordinates[2], 0)));
#endif
