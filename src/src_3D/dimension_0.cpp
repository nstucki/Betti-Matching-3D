...
RepresentativeCycle
Dimension0::getRepresentativeCycle(const Pair &pair,
    ...
    Coordinate vertex;
    parentIdx0 = uf.find(pair.birth.x() * cgc.n_yz +
                         pair.birth.y() * cgc.shape[2] + pair.birth.z());
    for (size_t i = 0; i < cgc.getNumberOfCubes(0); ++i) {
        parentIdx1 = uf.find(i);
        if (parentIdx0 == parentIdx1) {
            vertex = uf.getCoordinates(i);
            if (find(reprCycle.begin(), reprCycle.end(), vertex) ==
                reprCycle.end()) {
...
Dimension0::getAllRepresentativeCycles(uint8_t input, bool computeMatchedCycles,
                                       bool computeUnmatchedCycles) {
    const CubicalGridComplex &cgc = (input == 0) ? cgc0 : cgc1;
    UnionFind uf(cgc);
    vector<Pair> &pairs = (input == 0) ? pairs0 : pairs1;
    unordered_map<index_t, Pair> &matchMap =
    ...
    // Track the next pair to be found (we will find them in the same order as
    // before) to know when to save a cycle
    ...
    for (Cube &edge : edges) {
        ...
        if (parentIdx0 != parentIdx1) {
            ...
            if (currentPair != pairs.end() &&
                edge.index == currentPair->death.index) {
                currentPair++;
            } else {
    ...
    if (computeMatchedCycles) {
        ...
        for (auto &match : matches) {
            ...
            if (!unionFindIdx.has_value()) {
                throw runtime_error(
                    "Union find index for matched pair cannot be found");
            }
    ...
    if (computeUnmatchedCycles) {
        ...
        for (auto &pair : pairs) {
            if (!isMatched[pair.birth.index]) {
                auto unionFindIdx =
                    unionFindIdxByDeathCube.find(pair.death.index);
                if (!unionFindIdx.has_value()) {
                    throw runtime_error(
                        "Union find index for matched pair cannot be found");
                }
    ...
    for (Cube &edge : edges) {
        ...
        if (parentIdx0 != parentIdx1) {
            ...
            if (birth != edge.birth) {
                ...
                pairsComp.push_back(
                    Pair(Cube(birth, birthCoordinates[0], birthCoordinates[1],
