...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (arg == "--matched" || arg == "-m") { matchedFilename = string(argv[++i]); } 
        else if (arg == "--unmatched_0" || arg == "-u0") { unmatched0Filename = string(argv[++i]); } 
        else if (arg == "--unmatched_1" || arg == "-u1") { unmatched1Filename = string(argv[++i]); } 
        else {
            if (filename0.empty()) { filename0 = argv[i]; } 
            else { filename1 = argv[i]; }
		}
	}

    if (filename0.empty()) { print_usage_and_exit(-1); } 
    if (filename0.find(".txt")!= string::npos) { format0 = PERSEUS; } 
    else if (filename0.find(".npy")!= string::npos) { format0 = NUMPY; } 
    else if (filename0.find(".complex")!= string::npos) { format0 = DIPHA; } 
    else {
		cerr << "unknown input file format! (the filename extension should be .txt/.npy/.complex): " << filename0 << endl;
		exit(-1);
	}
    ...
    if (filename1.empty()) { print_usage_and_exit(-1); }
    if (filename1.find(".txt")!= string::npos) { format1 = PERSEUS; } 
    else if (filename1.find(".npy")!= string::npos) { format1 = NUMPY; } 
    else if (filename1.find(".complex")!= string::npos) { format1 = DIPHA; } 
    else {
		cerr << "unknown input file format! (the filename extension should be .txt/.npy/.complex): " << filename1 << endl;
		exit(-1);
	}
	...
	BettiMatching BM(std::move(input0), std::move(input1), std::move(shape), std::move(config) );
	BM.computeMatching();

	if (print) { BM.printResult(); }
	if (saveResult) {}
}