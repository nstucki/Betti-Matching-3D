...
BettiMatching::getMatchedRepresentativeCycles(const size_t &dim,
                                              const size_t &index) {
    if (!computed) {
        throw std::runtime_error("Betti Matching not computed yet");
    }
    ...
    default: {
        throw runtime_error("Computing matched representative cycles is only "
                            "supported for 1D, 2D and 3D volumes");
    }
    }
}
...
BettiMatching::computeAllRepresentativeCycles(const int input, const int dim,
    ...
    case 3: {
        dim3::BettiMatching &bettiMatching = std::get<dim3::BettiMatching>(
            dimensionSpecificBettiMatching.value());
        return bettiMatching.computeAllRepresentativeCycles(
            input, dim, computeMatchedCycles, computeUnmatchedCycles);
    }
    default: {
        throw runtime_error("Computing all representative cycles is only "
                            "supported for 3D volumes");
    }
    }
}
...
BettiMatching::getUnmatchedRepresentativeCycle(const uint8_t &input,
                                               const size_t &dim,
                                               const size_t &index) {
    if (!computed) {
        throw std::runtime_error("Betti Matching not computed yet");
